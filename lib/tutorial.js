...
(function () {
  ...
  const HINTS = {
    ...
    dino: {
      emoji: "🦖",
      text: "Tap or drag: soap, shower, towel!",
    },
    garden: {
      emoji: "🌱",
      text: "Tap a pot, then tap the can and the pot!",
    },
    scene: {
      emoji: "🖼️",
    ...
    farm: {
      emoji: "🏡",
      text: "Tap a tool, then tap an animal!",
    },
    icecream: {
      emoji: "🍦",
      text: "Tap a flavor to scoop it, then tap Eat!",
    },
    train: {
      emoji: "🚂",
  ...
  function buildOverlay(gameId, hint) {
    ...
    const scale = (window.Lawson && window.Lawson.paceScale) ? window.Lawson.paceScale() : 1;
    setTimeout(dismiss, 3800 * scale);
  }
