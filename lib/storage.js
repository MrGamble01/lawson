...
// IMPORTANT: only touches keys that end in "Best" (per-game high scores).
// Don't nuke "kidName", "dark", "lastDay", "streak", or the mute
// settings — the button is labelled "Reset scores", not "wipe profile".
// Stickers get reset separately via resetStickers().
function resetAllHighScores() {
  try {
    const removeKeys = [];
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && k.startsWith(STORAGE_PREFIX) && k.endsWith("Best")) {
        removeKeys.push(k);
      }
    }
