...
const STICKERS = [
  ...
  // Big / Small
  { id: "sizeWiz",      emoji: "📏", title: "Size Wiz",       desc: "8 big-or-small in a row!" },
  // Piano
...
function _updateStickerTileCount() {
  ...
  if (!el) return;
  const earned = _loadEarned();
  el.textContent = `${earned.size}/${STICKERS.length}`;
}
...
function _allStickersCelebration() {
  ...
  // Fireworks: scatter sparkles around the screen edges in waves.
  try {
    if (typeof window !== "undefined" && window.Lawson && typeof window.Lawson.sparkleAt === "function") {
      for (let wave = 0; wave < 5; wave++) {
        for (let i = 0; i < 8; i++) {
          setTimeout(() => window.Lawson.sparkleAt(
