...
function _updateStickerTileCount() {
  ...
  el.setAttribute("aria-label", `${earned.size} of ${STICKERS.length} stickers`);
...
function _showStickerToast(s) {
  ...
  // Speak the unlock so the kid hears why something just appeared — once
  // whatever is being said (usually the cheer that earned it) has
  // finished. Games award stickers a moment before they speak the cheer,
  // and a direct say() here was cut off by it in the same tick.
  try {
    const announce = () => { try { if (typeof say === "function") say(`Sticker! ${s.title}!`, 1.05); } catch (_) {} };
    if (typeof afterSpeech === "function") afterSpeech(announce, { beatMs: 150, minMs: 0, maxMs: 6000 });
    else announce();
  } catch (_) {}
