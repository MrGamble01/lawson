...
// Storyteller speed, a multiplier on every line's rate: 0.85 slower for a
// kid still learning the words, 1 normal, 1.15 faster. Saved on the device.
const SPEECH_RATES = { slower: 0.85, normal: 1, faster: 1.15 };
let _speechSpeed = "normal";
try {
  const stored = localStorage.getItem("lawson:speechSpeed");
  if (stored && SPEECH_RATES[stored]) _speechSpeed = stored;
} catch (_) {}
function setSpeechSpeed(speed) {
  if (!SPEECH_RATES[speed]) return;
  _speechSpeed = speed;
  try { localStorage.setItem("lawson:speechSpeed", speed); } catch (_) {}
}
function getSpeechSpeed() { return _speechSpeed; }
// How the storyteller says the kid's name. Speech engines mangle some
// names ("Siobhan", "Xavier", "Nguyen"); a parent can write how it
// sounds ("Shi-vawn") and every line that names the kid is spoken that
// way, while captions keep the written name. Saved on the device.
let _nameWritten = "";
let _nameSpoken = "";
try { _nameSpoken = (localStorage.getItem("lawson:nameSpoken") || "").trim(); } catch (_) {}
function setSpokenName(written, spoken) {
  _nameWritten = String(written == null ? "" : written).trim();
  _nameSpoken = String(spoken == null ? "" : spoken).trim();
  try {
    if (_nameSpoken) localStorage.setItem("lawson:nameSpoken", _nameSpoken);
    else localStorage.removeItem("lawson:nameSpoken");
  } catch (_) {}
}
function getSpokenName() { return _nameSpoken; }
// The text the engine should be given for a line: the written name swapped
// for its spoken form, whole words only ("Al" must not touch "Always").
function spokenForm(text) {
  const s = String(text);
  if (!_nameWritten || !_nameSpoken || _nameSpoken === _nameWritten) return s;
  const escaped = _nameWritten.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  // A replacer function: the spoken form is plain text, so a "$" in it is
  // never read as a replacement pattern ($&, $1, $`).
  return s.replace(new RegExp(`(^|[^\\p{L}\\p{N}])${escaped}(?![\\p{L}\\p{N}])`, "gu"), (_, pre) => pre + _nameSpoken);
}
function speechRateFor(rate) {
  // Engines accept 0.1–10 but sound wrong outside a narrow band; keep the
  // storyteller between "slow and clear" and "brisk".
  return Math.min(1.6, Math.max(0.6, rate * SPEECH_RATES[_speechSpeed]));
}
...
// ---------- Speech diagnostics ----------
// What actually happened to the last few utterances: how long the engine
// took to start, how the line ended, whether it was re-read or handed to
// a stand-in voice. This is how the watchdog and beat timings (chosen
// without an iPad to hand) get checked on the real device: Settings →
// "Listen to this voice" reports it. Ring buffer, newest last.
const SPEECH_LOG_MAX = 20;
const _speechLog = [];
let _currentEntry = null;
function _logAttempt(entry) {
  _speechLog.push(entry);
  if (_speechLog.length > SPEECH_LOG_MAX) _speechLog.shift();
  _currentEntry = entry;
}
function _finishAttempt(entry, outcome) {
  if (!entry || entry.outcome !== "pending") return;
  entry.outcome = outcome;
  entry.endMs = Date.now() - entry.t0;
}
// Entries: { text, voice, local, stage, delayMs, t0, startMs, endMs,
// outcome } with outcome one of pending | ended | cut | dropped |
// error:<code>. stage 0 first try, 1 same-voice re-read, 2 stand-in voice.
function speechLog() { return _speechLog.map(e => Object.assign({}, e)); }
function speechStats() {
  const done = _speechLog.filter(e => e.outcome !== "pending");
  const starts = done.filter(e => e.startMs != null).map(e => e.startMs).sort((a, b) => a - b);
  return {
    lines: done.length,
    started: starts.length,
    startMedianMs: starts.length ? starts[Math.floor(starts.length / 2)] : null,
    startMaxMs: starts.length ? starts[starts.length - 1] : null,
    ended: done.filter(e => e.outcome === "ended").length,
    cut: done.filter(e => e.outcome === "cut").length,
    dropped: done.filter(e => e.outcome === "dropped").length,
    errors: done.filter(e => /^error:/.test(e.outcome)).length,
    reread: done.filter(e => e.stage === 1).length,
    fallbacks: done.filter(e => e.stage === 2).length,
  };
}
...
function afterSpeech(fn, opts) {
  ...
  let timer = setTimeout(ceiling, maxMs);
  // `force` is the ceiling: fire no matter what. The beat re-checks first:
  // a line that started during it (a sticker announcement, "New best!")
  // gets heard too, with the ceiling still bounding the wait.
  function fire(force) {
    if (done) return;
    if (!force && _settleSpeech) {
      clearTimeout(timer);
      timer = setTimeout(ceiling, Math.max(0, maxMs - elapsed()));
      idle().then(() => {
        if (done) return;
        clearTimeout(timer);
        timer = setTimeout(fire, Math.min(beatMs, Math.max(0, maxMs - elapsed())));
      });
      return;
    }
    done = true;
  ...
  idle().then(() => {
    if (done) return;
    clearTimeout(timer);
    timer = setTimeout(fire, Math.max(beatMs, minMs - elapsed()));
  });
...
function cancelSpeech() {
  ...
  _settleSpeech = null;
  if (settle) settle();
  _finishAttempt(_currentEntry, "cut"); // in case the engine reports nothing for the cancel
}
...
const SPEECH_START_TIMEOUT_MS = 4000;       // remote voice: a network round trip
const LOCAL_START_TIMEOUT_MS = 2500;        // local voice: iOS warms the engine up in ~1 s
// iOS Safari and Android Chrome sometimes swallow an utterance that is
// spoken in the same tick as cancel() — no start, no error, nothing.
// Giving the engine a moment after a cut-off avoids most of it; the
// start watchdog above catches the rest.
const SPEAK_AFTER_CANCEL_MS = 60;
const SPEAK_AFTER_EFFECT_MAX_MS = 500;
// When the most recent beep/chime finishes (Date.now() ms). Set by beep(),
// which every chime helper is built from.
let _effectUntil = 0;
...
function say(text, rate = 0.95, caption) {
  // Anything new being said means the kid is busy or the round moved on:
  // a pending prompt reminder is no longer wanted.
  cancelPromptRepeat();
  if (_hidden) return Promise.resolve();
  // What the engine is given: the kid's name in its spoken form, if the
  // parent set one. Captions show the written line.
  const spokenText = spokenForm(text);
  ...
  if (typeof CustomEvent === "function") {
    window.dispatchEvent(new CustomEvent("lawson:say", {
      detail: { text: String(caption != null ? caption : text), spoken: spokenText, rate: speechRateFor(rate) },
    }));
  }
  if (!synth || _voiceMuted || getVolume() === 0) return Promise.resolve();
  // Cutting off a line that is still being said is exactly when the
  // engine may swallow the next one; give it a beat in that case.
  const busy = !!_settleSpeech || !!synth.speaking || !!synth.pending;
  // A chime that has just been triggered (happySound() before a cheer,
  // buzzSound() before "try again") would mask the first word; start the
  // line as the effect rings out instead. Capped so a long effect can
  // never hold a line back noticeably.
  const effectMs = Math.min(SPEAK_AFTER_EFFECT_MAX_MS, Math.max(0, _effectUntil - Date.now()));
  cancelSpeech();
  recoverSpeech();
  _speechDone = new Promise((resolve) => {
    ...
    // Speak the line with `voice`. Stages: 0 first try, 1 the same voice
    // again after the engine swallowed the utterance, 2 the fallback
    // voice after the voice itself failed. A retryable error marks the
    // voice unusable and moves to whatever pickVoice() now prefers. A
    // local voice that never starts is read again with the same voice
    // once (the engine hiccupped, not the voice); a remote one that
    // never starts goes straight to the fallback (#15). Events from an
    // attempt that has been superseded — or from a line that a later
    // say() cut off — are ignored, so a hung utterance being cancelled
    // can't end the retry.
    const speakWith = (voice, stage, delayMs) => {
      const mine = ++attempt;
      const u = new SpeechSynthesisUtterance(spokenText);
      u.rate = speechRateFor(rate);
      u.pitch = 1;
      u.volume = getVolume();
      ...
      const fail = (code) => {
        ...
        let next = null;
        if (voice && stage < 2) {
          if (code === "dropped" && voice.localService) {
            if (stage === 0) next = voice;
          } else if (code === "dropped" || RETRYABLE_SPEECH_ERRORS.test(code)) {
            markVoiceUnusable(voice.voiceURI);
            if (preferredVoice && preferredVoice.voiceURI !== voice.voiceURI) next = preferredVoice;
          }
        }
        if (next) {
          ...
          return speakWith(next, next === voice ? 1 : 2, SPEAK_AFTER_CANCEL_MS);
        }
        settle();
      };
      const entry = { text, voice: voice ? voice.name : "", local: voice ? !!voice.localService : null,
        stage, delayMs: delayMs || 0, t0: 0, startMs: null, endMs: null, outcome: "pending" };
      u.onstart = () => {
        if (mine !== attempt) return;
        clearTimeout(watchdog);
        watchdog = null;
        entry.startMs = Date.now() - entry.t0;
      };
      u.onend = () => { if (mine === attempt) { _finishAttempt(entry, "ended"); settle(); } };
      u.onerror = (e) => {
        const code = e && e.error;
        _finishAttempt(entry, code === "interrupted" || code === "canceled" ? "cut" : "error:" + code);
        fail(code);
      };
      const go = () => {
        if (mine !== attempt || _settleSpeech !== settle) return; // superseded or cut off meanwhile
        entry.t0 = Date.now();
        _logAttempt(entry);
        // Armed even with no voice to name (no English voice installed):
        // nothing to retry with then, but the line must still settle.
        watchdog = setTimeout(() => { _finishAttempt(entry, "dropped"); fail("dropped"); },
          voice && !voice.localService ? SPEECH_START_TIMEOUT_MS : LOCAL_START_TIMEOUT_MS);
        synth.speak(u);
      };
      if (delayMs) setTimeout(go, delayMs); else go();
    };
    speakWith(preferredVoice, 0, Math.max(busy ? SPEAK_AFTER_CANCEL_MS : 0, effectMs));
  });
  return _speechDone;
}

// ---------- Prompts that repeat once ----------
// A quiz prompt ("Find the cow!") is said once and then the game waits.
// A toddler who looked away mid-round hears nothing more. sayPrompt()
// says the line and, if nothing else has been said for a while, says it
// once more. Any later say() — the kid answered, a new round, a poke —
// drops the reminder, as does leaving the screen, a screen lock, or a
// muted voice.
const PROMPT_REPEAT_MS = 12000;
let _promptRepeat = null;
function sayPrompt(text, rate, caption, repeatMs = PROMPT_REPEAT_MS) {
  const spoken = say(text, rate, caption);   // say() drops any older reminder
  const mine = { timer: null };
  _promptRepeat = mine;
  mine.timer = setTimeout(() => {
    if (_promptRepeat !== mine) return;
    _promptRepeat = null;
    if (_hidden || _voiceMuted || getVolume() === 0 || _settleSpeech) return;
    say(text, rate, caption);
  }, repeatMs);
  return spoken;
}
function cancelPromptRepeat() {
  if (!_promptRepeat) return;
  clearTimeout(_promptRepeat.timer);
  _promptRepeat = null;
}
...
try {
  ...
  // A corrupt saved value keeps the default rather than muting.
  if (Number.isFinite(n)) _volume = Math.max(0, Math.min(1, n));
} catch (_) {}
masterGain.gain.value = _volume;
masterGain.connect(audioCtx.destination);

// Music bus: the menu loop goes through its own gain on the way to the
// master, so the storyteller can duck it. A hub welcome, a Bobo tickle
// or the Settings voice preview is said over the loop, and at full level
// the melody sits on the words. While a line is in flight the music
// drops to a third of its level; once the line has ended (or been cut
// off) it eases back up. Ramps are scheduled on the audio clock, so a
// cut-off followed at once by the next line never flickers.
...
function _duckMusic(on) {
  on = !!on;
  if (on === _musicDucked) return;
  _musicDucked = on;
  const target = on ? MUSIC_DUCK : 1;
  const g = musicGain.gain;
  try {
    const t = audioCtx.currentTime || 0;
    g.cancelScheduledValues(t);
    g.setValueAtTime(g.value, t);
    g.linearRampToValueAtTime(target, t + (on ? DUCK_DOWN_S : DUCK_UP_S));
  } catch (_) {
    try { g.value = target; } catch (_) {}
  }
}
function isMusicDucked() { return _musicDucked; }
...
function beep(freq = 440, dur = 0.15, type = "sine", when = 0) {
  if (_soundMuted) return;
  _effectUntil = Math.max(_effectUntil, Date.now() + Math.round((when + dur) * 1000));
  try {
